...
<body>
    <div class="container">
        ...
        <!-- Navigation -->
        <div class="nav-tabs">
            <button class="nav-tab active" onclick="showTab('scorecard')">
                🏆 Live Scorecard
            </button>
            <button class="nav-tab" onclick="showTab('live-data')">
                🌐 Live Cricket Data
            </button>
            <button class="nav-tab" onclick="showTab('players')">
                👥 Player Management
            </button>
        ...
        <!-- Live Cricket Data Tab -->
        <div id="live-data" class="tab-content">
            <div class="management-section">
                <div class="section-header">
                    <h2 class="section-title">Live Cricket Data from API</h2>
                    <button class="add-btn" onclick="syncLiveData()">
                        🔄 Sync Live Data
                    </button>
                </div>

                <div class="form-buttons">
                    <button class="btn" style="background: #9C27B0;" onclick="testCricketAPI()">
                        🧪 Test API Connection
                    </button>
                    <button class="btn btn-save" onclick="loadCurrentMatches()">
                        🔴 Current Matches
                    </button>
                    <button class="btn btn-edit" onclick="loadRecentMatches()">
                        📅 Recent Matches
                    </button>
                    <button class="btn" style="background: #FF9800;" onclick="loadUpcomingMatches()">
                        ⏰ Upcoming Matches
                    </button>
                </div>
                ...
                <div id="live-data-container" class="data-table">
                    <div class="loading">Click on any button above to load live cricket data from CricketData.org API</div>
                </div>
            </div>
        </div>
        ...
        <div id="players" class="tab-content">
            <div class="management-section">
                <div class="section-header">
                    ...
                    <button class="add-btn" onclick="showAddForm('player')">
                        ➕ Add Player
                    </button>
                </div>
                ...
                <div id="player-form" class="form-container">
                    ...
                    <div class="form-buttons">
                        <button class="btn btn-save" onclick="savePlayer()">
                            💾 Save Player
                        </button>
                        <button class="btn btn-cancel" onclick="hideAddForm('player')">
                            ❌ Cancel
                        </button>
                    </div>
                </div>
                ...
                <div class="data-table">
                    <table class="table" id="players-table">
                        ...
                        <tbody id="players-tbody">
                            <tr>
                                <td colspan="8" class="loading">Loading players...</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div id="matches" class="tab-content">
            <div class="management-section">
                <div class="section-header">
                    ...
                    <button class="add-btn" onclick="showAddForm('match')">
                        ➕ Add Match
                    </button>
                </div>
                ...
                <div id="match-form" class="form-container">
                    ...
                    <div class="form-buttons">
                        <button class="btn btn-save" onclick="saveMatch()">
                            💾 Save Match
                        </button>
                        <button class="btn btn-cancel" onclick="hideAddForm('match')">
                            ❌ Cancel
                        </button>
                    </div>
                </div>
                ...
                <div class="data-table">
                    <table class="table" id="matches-table">
                        ...
                        <tbody id="matches-tbody">
                            <tr>
                                <td colspan="8" class="loading">Loading matches...</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        ...
        async function testCricketAPI() {
            try {
                ...
                if (response.status === 'success') {
                    ...
                    resultDiv.innerHTML = '<h3>✅ API Connection Successful!</h3><p>' + response.message + '</p>';
                } else {
                    resultDiv.className = 'error';
                    resultDiv.innerHTML = '<h3>❌ API Connection Failed</h3><p>' + response.message + '</p>';
                }
        ...
        async function loadCurrentMatches() {
            try {
                ...
                matches.forEach(function(match) {
                    ...
                    statusCell.innerHTML = '<span class="status-badge status-upcoming">UPCOMING</span>';
                    ...
                    seriesCell.textContent = match.series || 'N/A';
                    ...
                    detailsBtn.onclick = function() { viewMatchDetails(match.id); };
                ...
                document.getElementById('live-data-container').innerHTML = 
                    '<div class="error">Failed to load upcoming matches</div>';
            }
        }
        ...
        async function viewMatchDetails(matchId) {
            try {
                const response = await apiRequest('/live/match/' + matchId + '/details');
                ...
                modalContent.style.cssText = 'background: white; color: #333; padding: 30px; border-radius: 15px; max-width: 90%; max-height: 80%; overflow-y: auto;';
                
                const title = document.createElement('h2');
                title.textContent = 'Match Details';
                title.style.cssText = 'margin-bottom: 20px; color: #764ba2;';
                
                const dataDisplay = document.createElement('div');
                dataDisplay.style.cssText = 'background: #f5f5f5; padding: 20px; border-radius: 8px; margin-bottom: 20px; max-height: 400px; overflow-y: auto;';
                
                if (matchData && Object.keys(matchData).length > 0) {
                    const pre = document.createElement('pre');
                    pre.style.cssText = 'white-space: pre-wrap; font-family: monospace; font-size: 14px;';
                    pre.textContent = JSON.stringify(matchData, null, 2);
                    dataDisplay.appendChild(pre);
                } else {
                    dataDisplay.textContent = 'No detailed match data available';
                }
                
                const closeButton = document.createElement('button');
                closeButton.textContent = 'Close';
                closeButton.style.cssText = 'background: #f44336; color: white; border: none; padding: 12px 24px; border-radius: 5px; cursor: pointer;';
        ...
        async function syncLiveData() {
            try {
                showMessage('Syncing live cricket data to database...', 'success');
                ...
                container.innerHTML = 
                    ...
                    '<div class="current-batters">' +
                        ...
                        (liveData.current_batters && liveData.current_batters.length > 0 ? 
                            liveData.current_batters.map(function(batter) {
        ...
        async function loadAnalytics() {
            try {
                ...
                if (analytics.top_run_scorers && analytics.top_run_scorers.length > 0) {
                    analytics.top_run_scorers.slice(0, 4).forEach(function(player) {
                        analyticsHTML += 
        ...
        async function loadPlayers() {
            try {
                ...
                players.forEach(function(player) {
                    ...
                    editBtn.onclick = function() { editPlayer(player.id); };
                    
                    const deleteBtn = document.createElement('button');
                    deleteBtn.className = 'btn btn-delete';
                    deleteBtn.textContent = '🗑️ Delete';
                    deleteBtn.onclick = function() { deletePlayer(player.id); };
        ...
        async function loadMatches() {
            try {
                ...
                matches.forEach(function(match) {
                    ...
                    // Basic cells
                    const cells = [match.team1, match.team2, match.score1, match.score2];
                    cells.forEach(function(cellData) {
                        const cell = document.createElement('td');
                        cell.textContent = cellData;
                        row.appendChild(cell);
                    ...
                    // Remaining cells
                    const remainingCells = [match.overs, match.venue || 'TBA'];
                    remainingCells.forEach(function(cellData) {
                        const cell = document.createElement('td');
                        cell.textContent = cellData;
                        row.appendChild(cell);
                    });
                    ...
                    editBtn.className = 'btn btn-edit';
                    editBtn.textContent = '✏️ Edit';
                    editBtn.onclick = function() { editMatch(match.id); };
                    
                    const deleteBtn = document.createElement('button');
                    deleteBtn.className = 'btn btn-delete';
                    deleteBtn.textContent = '🗑️ Delete';
                    deleteBtn.onclick = function() { deleteMatch(match.id); };
        ...
        async function loadRecentMatches() {
            try {
                const container = document.getElementById('live-data-container');
                container.innerHTML = '<div class="loading">Loading recent matches...</div>';
                
                const response = await apiRequest('/live/recent-matches');
                const matches = response.data;
                ...
                const table = createTable(['Match', 'Status', 'Teams', 'Result', 'Venue', 'Date', 'Actions']);
                const tbody = table.querySelector('tbody');
                ...
                matches.forEach(function(match) {
                    ...
                    resultCell.textContent = match.result || 'Result TBA';
                ...
                document.getElementById('live-data-container').innerHTML = 
                    '<div class="error">Failed to load recent matches</div>';
            }
        }
        ...
        async function loadUpcomingMatches() {
            try {
                const container = document.getElementById('live-data-container');
                container.innerHTML = '<div class="loading">Loading upcoming matches...</div>';
                
                const response = await apiRequest('/live/upcoming-matches');
                const matches = response.data;
                
                if (matches.length === 0) {
                    container.innerHTML = '<div class="loading">No upcoming matches found</div>';
                    return;
                }
                
                const table = createTable(['Match', 'Status', 'Teams', 'Date', 'Venue', 'Series', 'Actions']);
                const tbody = table.querySelector('tbody');
